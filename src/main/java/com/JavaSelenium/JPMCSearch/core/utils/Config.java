...
public class Config {
    ...
    private static void readConfigContent() throws FrameworkExceptions {
        Logs.trace("Executing the function to read the Config.xml file content mentioned as:" + Constants.CONFIGFILEPATH);
    ...
    public static String getConfigProperty(String configName) throws FrameworkExceptions {
        ...
        Logs.trace("Fetching the value from the Configuration file with key '" + configName + "' and the result is '" + configValue + "'");
        if (null == configValue) {
