...
public class Components {
    ...
    public <E extends IElement> E getComponent(final WebElement element, Class<E> elementClass, Class referenceClass) {
        try {
            Waits.waitForPageLoadJS();
            WebElement waitElementStatus = Waits.componentWait(element, Integer.parseInt(Config.getConfigProperty(Constants.ELEMENTWAITTIME)));
            ...
            Logs.error("Failed in getComponents() method of Components class. Element: "+getVariableCausedException(referenceClass, element) + fe);
        }
        return null;
    }
